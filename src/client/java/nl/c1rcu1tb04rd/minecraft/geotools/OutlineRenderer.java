...
public final class OutlineRenderer {
    ...
    public static void render(
        ...
        if (
            (green == null || green.isEmpty()) && (red == null || red.isEmpty())
        ) return;
        ...
        VertexConsumer vc = consumers.getBuffer(
            RenderLayers.secondaryBlockOutline()
        );

        if (red != null) {
            for (BlockPos p : red)
                drawBlockOutline(matrices, vc, p, 0x80FF0000); // 50% alpha red
        }
        if (green != null) {
            for (BlockPos p : green)
                drawBlockOutline(matrices, vc, p, 0x8000FF00); // 50% alpha green
        }
